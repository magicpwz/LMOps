...
class BaseTask(object):
    ...
    def get_dataset_valid(self, data):
        ...
        if len(res) > 2000:
            res = res.select([i for i in range(2000)])
        return res
    ...
    def get_data_for_fewshot(self):
        ...
        for i in range(len(labels) // self.class_num):
            for j in range(i*self.class_num, (i+1)*self.class_num):
                ...
                context_len.append(len(context_src_tokens))
        
        return src_tokens_valid, gpt_loss_mask_valid, labels, self.answer_set, context_len


    def get_data_for_ft(self):
        src_tokens_train, gpt_loss_mask_train, _, _ = self.tokenized_data('train')
        ...
        if self.permut_index > 0:
            for i in range(5):
                permutation_set.append(np.random.permutation(min_num))
            src_tokens_train = src_tokens_train[permutation_set[self.permut_index]]
            gpt_loss_mask_train = gpt_loss_mask_train[permutation_set[self.permut_index]]
        
        # multi-sequence fine-tune
        for i in range(len(src_tokens_train)):
            src_tokens_train[i] = src_tokens_train[i][:-1]
            gpt_loss_mask_train[i] = gpt_loss_mask_train[i][:-1]
        context_src_tokens = src_tokens_train
        gpt_loss_mask = gpt_loss_mask_train
        true_k = len(context_src_tokens)
        
        print(f"NOTE: true K of baseline is {true_k}")

        return context_src_tokens, gpt_loss_mask
        ...
        for i in range(min_num):
            ...
            if split == 'train':
                ...
                if label is None:
                    continue
                if i < 2:
                    print(f"------------ example {i} input str train is {input_str}")
                    print(f"------------ example {i} label str train is {label_str}")
                ...
                gpt_loss_mask.append([True]*cur_length)
                labels.append(label)
                ...
                if i < 2:
                    print(f"------------ example {i} input str valid is {input_str}")
                    print(f"------------ example {i} label str valid is {label_str}")
